...
class Monster:
    probability = 0.15  # 15% 的概率在生成新平台时产生怪兽

    def __init__(self, y):
        # 1. 加载 5 帧动画
        self.frames = [Utils.load_img(f"blue_flying_monster_{i}.png", (50, 40)) for i in range(1, 6)]
        self.current_frame = 0
        self.anim_timer = 0
        self.anim_speed = 0.2  # 动画切换速度
        ...
        # 3. 设置飞行速度（左右随机）
        self.speed_x = random.uniform(1.5, 3.5) * random.choice([-1, 1])
...
class GameSession:
    def __init__(self):
        ...
        self.monsters = []
        self.buzzer_sound = Utils.load_sound("buzzer")
    ...
    def update_scroll(self):
        ...
        while len(self.platforms) < 8: # 保持屏幕上至少有8个平台
            ...
            new_platform = Platform(random.randint(0, SCREEN_WIDTH - 70), new_y)
            self.generate_item(new_platform)
            self.platforms.append(new_platform)
            if Utils.hit_probability(Monster.probability):
                self.monsters.append(Monster(new_y - 60)) # 生成在平台上方一点
    ...
    def update(self): # 更新游戏数据
        ...
        for monster in self.monsters:
            monster.animate()
            monster.update(0) # 这里传0是因为 update_scroll 已经处理过滚动了
